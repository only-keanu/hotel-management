...
import { Room, Guest, Booking } from '../../types/types';
...
const BookingFormModal: React.FC<BookingFormModalProps> = ({
  ...
  // ✅ FIXED: Ensure rooms and guests are always arrays
  const safeRooms = Array.isArray(rooms) ? rooms : [];
  const safeGuests = Array.isArray(guests) ? guests : [];

  // Filter available rooms
  const availableRooms = safeRooms.filter(
      r => r.status === 'available' || (booking && r.id === booking.roomId)
  );

  // ✅ FIXED: Use safeRooms instead of rooms
  const selectedRoom = safeRooms.find(r => r.id === roomId);
  ...
  const validateForm = () => {
    ...
    if (selectedRoom && selectedRoom.capacity && adults + children > selectedRoom.capacity) {
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                ...
                <input
                    ...
                    onChange={e => setAdults(Math.min(parseInt(e.target.value) || 1, selectedRoom?.capacity || 10))}
                    className={`w-full p-2 border rounded-md ${
                        errors.adults || errors.capacity ? 'border-red-500' : 'border-gray-300'
                    }`}
                />
              ...
              <div className="flex-1">
                ...
                <input
                    ...
                    value={children}
                    min={0}
                    max={selectedRoom ? selectedRoom.capacity - adults : 9}
                    onChange={e =>
                        setChildren(Math.min(parseInt(e.target.value) || 0, selectedRoom ? selectedRoom.capacity - adults : 9))
                    }
                    className={`w-full p-2 border rounded-md ${
            ...
            {roomId && selectedRoom && (
                <div className="bg-gray-50 p-4 rounded-md">
                  ...
                  <div className="flex justify-between mt-2 text-sm text-gray-600">
                    <span>Nights:</span>
                    <span>{Math.max(differenceInDays(new Date(checkOutDate), new Date(checkInDate)), 0)}</span>
                  </div>
