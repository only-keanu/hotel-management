...
const BookingDetailModal: React.FC<BookingDetailModalProps> = ({
  ...
  return <>
      <Modal isOpen={isOpen} onClose={onClose} title="Booking Details" size="lg" footer={<>
            {booking.status === 'confirmed' && <button onClick={() => setShowCheckInConfirm(true)} className="px-4 py-2 text-sm bg-green-600 text-white rounded-md hover:bg-green-700">
                Check In
              </button>}
            {booking.status === 'checked_in' && <>
                ...
                <button onClick={() => setShowCheckOutConfirm(true)} className="px-4 py-2 text-sm bg-purple-600 text-white rounded-md hover:bg-purple-700">
                  Check Out
                </button>
              </>}
          </>}>
      ...
      <ConfirmationModal isOpen={showCheckInConfirm} onClose={() => setShowCheckInConfirm(false)} onConfirm={handleConfirmCheckIn} title="Confirm Check-in" message={`Are you sure you want to check in ${guest.firstName} ${guest.lastName} to Room ${room.number}?`} confirmText="Check In" confirmButtonClass="bg-green-600 hover:bg-green-700" />
      {/* Check-out Confirmation Modal */}
      <ConfirmationModal isOpen={showCheckOutConfirm} onClose={() => setShowCheckOutConfirm(false)} onConfirm={handleConfirmCheckOut} title="Confirm Check-out" message={`Are you sure you want to check out ${guest.firstName} ${guest.lastName} from Room ${room.number}?`} confirmText="Check Out" confirmButtonClass="bg-purple-600 hover:bg-purple-700" />
    </>;
};
